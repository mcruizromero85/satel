...
select * from encuentros
select e.id, r.numero ,ia.gamer_id ,ia.etiqueta_llave, ib.gamer_id, ib.etiqueta_llave, e.encuentro_anterior_a_id, e.encuentro_anterior_b_id from 
encuentros e, inscripciones ia, inscripciones ib, rondas r
where e.gamerinscritoa_id=ia.id and e.gamerinscritob_id=ib.id and r.id = e.ronda_id
select * from partidas where encuentro_id=441
delete from partidas where id=23
