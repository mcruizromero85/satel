...
describe TorneosHelper do
  ...
  it 'Transformar fechas de cierre de inscripciones mayores a 2 días a formato de cuenta regresiva, pero redondeado al mayor' do
    expect(TorneosHelper.obtenerFormatoCuentaRegresivaHastaLaFecha(Time.at(fecha_actual_con_milisegundos + (60 * 60 * 59)))).to eq "2d"
    expect(TorneosHelper.obtenerFormatoCuentaRegresivaHastaLaFecha(Time.at(fecha_actual_con_milisegundos + (60 * 60 * 66) ))).to eq "3d"
  end
